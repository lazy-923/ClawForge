...
当前 `MEMORY.md` 不是松散 bullet 列表，而是由结构化 memory records 组成。典型格式如下：
...
这样做让长期记忆既能保持 Markdown 可读性，又能作为独立检索单元参与 BM25 / vector retrieval。
...
提供 draft 列表、详情、promote / merge-preview / merge / ignore 操作。
...
当前实现已经从单纯阈值过滤升级为“两阶段注入决策”：

- 先由 retriever 返回 candidate skills
- 再由 selector 决定是否真正注入 Prompt
- 优先尝试 LLM 输出严格 JSON 的注入决策
- LLM 不可用或失败时，回退到原有 score 阈值规则

当前 selector 输出除了 `selected_skills` 外，还会补充：
...
这让前端和调试过程不只知道“命中了什么”，还知道“为什么注入/为什么不注入”。
...
长期记忆写入需要先进入 memory candidate 队列。低置信候选保持 `pending` 等待人工 promote；达到 `MEMORY_AUTO_PROMOTE_MIN_CONFIDENCE` 的高置信候选会自动晋升，写入结构化 memory record 并重建 memory index。
...
Memory 检索仍复用 `BaseHybridIndexStore`，但并不直接把整篇 `MEMORY.md` 当成普通文档切块。

当前 `backend/graph/memory_indexer.py` 会先执行：

```text
MEMORY.md
-> parse ### Memory: records
-> one memory record = one LlamaIndex Document
-> hybrid retrieval
```
...
如果旧版本 `MEMORY.md` 中仍存在简单 bullet 记忆，索引器会把它们兼容解析为 legacy records。
...
当前 Extractor 已升级为“LLM 主路径 + 规则 fallback”的结构：

- 当存在 repeated intent、identity match 或显式长期 skill signal 时，优先尝试 LLM 提取
- LLM 返回严格 JSON 的 `DraftCandidate` 结构
- 若 LLM 不可用、输出无效、置信度不足，则回退到原有启发式模板抽取
- 启发式路径仍然负责守住“单次一次性请求不应产生 draft”的保守边界
...
当前实现已经从“单轮关键词触发”升级为“LLM 抽取 + 多轮启发式保守兜底”，但还没有达到完整的 replay/eval 驱动演化形态。
...
当前 Judge 已升级为“LLM 主路径 + 规则 fallback”的治理决策器，核心逻辑包括：

- 先构造规则 fallback judgment
- 再让 LLM 在 related skills 证据上输出 `add / merge / ignore`
- 校验 LLM 输出是否合法、target 是否存在、confidence 是否达标
- 若不满足条件，则稳定回退到原有规则判断

当前 judgment 除 `action / target_skill / reason` 外，还可以补充：
...
1. 先通过 `build_merge_plan()` 读取目标 skill 文件并生成 merge plan
2. 解析 frontmatter 和正文结构
3. 提取原有 goal / constraints / workflow
4. 计算新增约束和新增 workflow
5. 执行 patch version bump
6. 生成新的 skill 文本与 `preview`
7. 生成结构化 `merge_patch`
8. `apply_merge_plan()` 在真正写文件前保存旧版本 snapshot
9. 再写入新的 skill 内容
...
- 支持先调用 `preview_merge()` 获取 merge plan
- 再调用 `merge_draft_into_skill()`
- 更新 draft 状态
- 刷新索引
...
memory dreaming 会自动生成 candidate；低置信候选保持 pending，高置信候选会自动晋升进 `MEMORY.md`，人工 promote 仍然可用。
...
当前三者都已有可运行实现，并且都已经从纯规则版走到了更接近治理器的形态：

- Extractor 已支持 LLM 主路径与规则 fallback
- Judge 已支持 LLM 主路径与规则 fallback
- Merger 已支持 preview、snapshot 与 rollback

当前最大的缺口转向：

- replay / eval 尚未接入治理链路
- promotion 仍没有质量门禁

## 10.3 测试与回放

系统已有可运行的 unittest 回归基础，当前后端关键路径已覆盖 memory/session、memory dreaming、retrieval、API smoke、governance、merge/versioning 等场景。
